...
The [trend-detector](https://github.com/modryn-studio/trend-detector) pipeline has been running every morning for a week now. It's spitting out a briefing with daily BUILD / WATCH / SKIP verdicts.
...
- **Sources.** trendspy is the PyPI package that calls Google's internal `trending_now()` API — 400+ rising topics per call. Also, that is NOT the unrelated French dropshipping tool. The source isn't the moat. The judgment layer is.
