...
const STATUS_LABEL: Record<ToolStatus, { label: string; className: string }> = {
  live: { label: "Live", className: "border-green-600/20 bg-green-500/10 text-green-600 dark:text-green-400" },
  beta: { label: "Beta", className: "border-blue-600/20 bg-blue-500/10 text-blue-600 dark:text-blue-400" },
  building: { label: "Building", className: "border-orange-600/20 bg-orange-500/10 text-orange-600 dark:text-orange-400" },
  "coming-soon": { label: "Coming Soon", className: "border-muted-foreground/20 bg-muted text-muted-foreground" },
};
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-24">
      ...
      <div className="mt-8">
        ...
        <p className="mt-4 font-mono text-sm leading-relaxed text-muted-foreground md:text-base">
          {tool.description}
        </p>

        {tool.status === "live" && tool.url && (
          <a href={tool.url} target="_blank" rel="noopener noreferrer">
            <Button className="mt-8 rounded-none bg-amber px-6 font-mono text-sm text-white hover:bg-amber/90">
              Try it
