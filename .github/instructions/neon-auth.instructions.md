...
**Important:** `NEXT_PUBLIC_SITE_URL` must be added to Vercel environment variables _before_ the deployment runs. If you add it after a deploy, trigger a fresh redeploy — the value is baked in at build time.
...
Neon Auth manages its own user store separately from your application tables.
